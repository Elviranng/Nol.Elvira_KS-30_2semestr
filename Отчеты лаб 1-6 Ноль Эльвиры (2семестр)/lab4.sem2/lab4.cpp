...
int main() {
	...
	string input = "Just take a look at this string ";
	SHA512 sha512;
	...
	for (int i = 0; i < 10; i++) {
		...
		output << "Размер строки: " << input.size() << ",\nсама строка: \"" << input << "\",\nвремя обработки: " << milli_diff.count() << " ìñ,\nхэш: " << hash << endl << endl;
		input += input;
	}

	
	string hash = sha512.hash("0T]Gk4GxsCCt83${#,7K{qTyF@VR}^+M}jeB 7{#M8f:#Xd:=d");
	output << hash << endl;
